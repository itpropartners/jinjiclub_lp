...
class CreateUsersTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('users', function (Blueprint $table) {
            ...
            $table->string('first_name_kana', 255)->default('')->comment('名(カナ)');
            $table->string('last_name_kana', 255)->default('')->comment('性(カナ)');
            $table->date('birthday')->nullable()->comment('生年月日');
            $table->unsignedTinyInteger('gender')->nullable()->comment('性別');
            $table->string('password', 255)->comment('パスワード');
            $table->string('email', 255)->comment('メールアドレス');
            $table->string('avatar_path', 255)->default('')->comment('アバター画像');
            ...
            $table->string('tel', 255)->default('')->comment('電話番号');
        ...
        // テーブルコメント設定
        DB::statement("ALTER TABLE `users` COMMENT '求職者'");
    }
